...
socket.on('check-online-login', function (userId) {
    ...
    $(`#online-conversation-${userId}`).addClass('fas fa-circle font-size-10 text-success d-inline-block ms-1 position-absolute');
...
socket.on('check-offline-logout', function (userId) {
    ...
    $(`#online-conversation-${userId}`).removeClass('fas fa-circle font-size-10 text-success d-inline-block ms-1 position-absolute')
...
$(`#btn-busy-${currentUserId}`).off('click').on('click', async function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                $('#btn-change-online').removeClass().addClass('fas fa-circle font-size-10 text-warning me-1 ms-0 position-absolute');
...
$(`#btn-online-${currentUserId}`).off('click').on('click', async function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                $('#btn-change-online').removeClass().addClass('fas fa-circle font-size-10 text-success me-1 ms-0 position-absolute');
                // socket.emit('busy', user);
            }
        },
    });
